...
import {NODES_STATE_VAR} from './storage/withApolloProvider'
...
//---------------------------------------
  ...
  //todo: trying to dynamically change node width based on contents...
  //UPDATE: but then remembered that the width we're talking about here is
  //actually the distance from the next node, not the width of the node

  let width = 0
  function getNodeWidth(node,i){
    //console.log("getNodeHeight",node);
    function round50(x) { return Math.ceil(x/50)*50; }
    if(node.name === "agg"){return 150}
    return node.data.length * 150
  }
  ...
  function getNodes(){
    ...
    globalState.node.forEach(n =>{n.width = 220;n.height = 150})
    return globalState.node
        .filter(n => n.data.length > 0)
        .filter(n => n.name !== 'agg')
        .map((data, i) => ({ ...data, x: (width += data.width) - data.width }))
  }
          ...
          <List>
            {transitions.map(({item, props: {x, ...rest}, key}, index) => (
                <animated.div
                    ...
                    style={{transform: x.interpolate(x => `translate3d(${x}px,0,0)`), zIndex: item.length, ...rest}}
                >
                  {/*<Node item={item}/>*/}
